...
import { IEntry, IHighlightedEntry, IMetaData, IIntroduction, IndividualEntrySortingEntry, UserRole } from "../types";
...
interface ICaseContext {
  ...
  setIndividualEntrySorting: Dispatch<SetStateAction<{ [key: string]: IndividualEntrySortingEntry[] }>>;
}
...
export const CaseProvider: React.FC<CaseProviderProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (Object.keys(individualEntrySorting).length === 0 && entries.length > 0) {
    ...
    } else if (Object.keys(individualEntrySorting).length > 0 && entries.length > 0) {
      ...
      const newEntries = entries.filter((entry) => !entryIdsInSorting.includes(entry.id));
      ...
      const entriesToRemove = getEntriesToRemove(individualEntrySorting, entryIdsInEntries);
      ...
      if (entriesToRemove.length > 0) {
        newIndividualEntrySorting = removeEntryIdsFromSorting(individualEntrySorting, entriesToRemove);
      }
 ...
 */
function getEntryIdsInSorting(individualEntrySorting: { [key: string]: IndividualEntrySortingEntry[] }): string[] {
...
function removeEntryIdsFromSorting(
  ...
  return Object.entries(individualEntrySorting).reduce((acc, [sectionId, sectionEntries]) => {
