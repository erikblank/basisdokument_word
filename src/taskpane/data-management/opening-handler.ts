import { IEntry, IndividualEntrySortingEntry, IStateUserInput, UserRole } from "../types";
import { v4 as uuidv4 } from "uuid";

export function updateSortingsIfVersionIsDifferent(basisdokumentObject: any, editFileObject: any) {
  ...
  individualEntrySortingFromEditFile = editFileObject["individualEntrySorting"] || {};
  ...
  entries.forEach((entry) => {
    ...
    if (
      !individualEntrySortingFromEditFile[entry.sectionId]?.some((individualEntry) =>
  ...
  individualEntrySortingFromEditFile = Object.keys(individualEntrySortingFromEditFile).reduce((acc, sectionId) => {
    const entrySorting: IndividualEntrySortingEntry[] = individualEntrySortingFromEditFile[sectionId];

    const entryIds = entries.filter((entry) => entry.sectionId === sectionId).map((entry) => entry.id);
