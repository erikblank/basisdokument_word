...
function getEntryTimestamp(childEntry: any, obj: any) {
  ...
  let timestamp = format(new Date(obj["versions"][childEntryVersion - 1].timestamp), "dd.MM.yyyy");
  return timestamp;
}
  ...
  if (entryId) {
    ...
    for (var i = 0; i < obj["entries"].length; i++) {
      if (obj["entries"][i].id === entryId) {
        title = obj["entries"][i].entryCode + " | Autor: " + obj["entries"][i].author;
      }
    }
...
async function downloadBasisdokumentAsPDF(
  ...
  let basicData = {
    ...
    timestamp: "Export: " + obj["versions"][obj["versions"].length - 1]["timestamp"].toLocaleString(),
  ...
  for (let i = 0; i < obj["sections"].length; i++) {
    ...
    titleEntry = {
      ...
      titleDefendant: "Titel Beklagtenpartei: " + obj["sections"][i].titleDefendant,
    };
  ...
  if (obj["regard"] === undefined) {
    basic = [[basisdokument[0].caseId], [basisdokument[0].version], [basisdokument[0].timestamp]];
  } else {
    basic = [
  ...
  for (let i = 0; i < allHints.length; i++) {
    ...
    autoTable(doc, {
      ...
      body: hintData,
      styles: {
        lineColor: allHints[i].version === obj["currentVersion"] ? [15, 82, 186] : 100,
    ...
    for (let i = 0; i < newEntries.length; i++) {
      ...
      if (newEntries[i].associatedEntry) {
        if (newEntries[i].evidences) {
          ...
          data = [["Neuer Beitrag"], [newEntries[i].title], [newEntries[i].associatedEntry], [newEntries[i].text]];
        }
    ...
    if (allEntries[i].section) {
      ...
      if (allEntries[i].associatedEntry !== undefined) {
        if (allEntries[i].evidences !== undefined) {
          ...
          data = [[allEntries[i].title], [allEntries[i].associatedEntry], [allEntries[i].text]];
        }
      ...
      autoTable(doc, {
        theme: "grid",
        styles: {
          lineColor: allEntries[i].version === obj["currentVersion"] ? [15, 82, 186] : 100,
          lineWidth: allEntries[i].version === obj["currentVersion"] ? 0.6 : 0.3,
        },
        body: data,
        margin: {
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    doc.text("Seite " + currentPageNum + "/" + pageCount, pageNumPos, pageHeight - 7);
  }
...
export function downloadBasisdokument(
  ...
  const date: Date = basisdokumentObject["versions"][basisdokumentObject["versions"].length - 1]["timestamp"];
